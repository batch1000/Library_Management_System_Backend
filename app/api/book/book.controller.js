...
async function addFavoriteBook(req, res) {
    try {
        ...
        if (!result) {
            console.log('Thêm sách vào yêu thích thất bại (đã tồn tại):', { MaSach: bookId, MaDocGia: readerId });
...
async function addRatingBook(req, res) {
    try {
        ...
        const result = await bookService.addRatingBook(bookId, readerId, stars, comment);

        if (!result) {
            console.log('Thêm đánh giá sách thất bại:', { MaSach: bookId, MaDocGia: readerId });
...
async function updateRatingComment(req, res) {
    try {
        ...
        const result = await bookService.updateRatingComment(bookId, readerId, comment);

        if (!result) {
            console.log('Cập nhật bình luận thất bại:', { MaSach: bookId, MaDocGia: readerId });
...
async function addBookView(req, res) {
    try {
        ...
        if (!result) {
            console.log("Ghi nhận lượt xem thất bại:", { MaSach: bookId, MaDocGia: readerId });
